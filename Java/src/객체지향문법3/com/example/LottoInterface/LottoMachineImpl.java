...
// 인터페이스를 구현하게 되면, 반드시 인터페이스가 가지고 있는 메소드를 오버라이딩 해줘야 한다.
public class LottoMachineImpl implements LottoMachine {
    ...
    public void mix() {
        for(int i = 0; i < 10000; i++) {
            ...
            if(x1 != x2) {
                Ball tmp = balls[LottoMachine.RETURN_BALL_COUNT]; // tmp : 값을 치환할 때는 같은 type의 임시변수가 필요하다
                balls[x1] = balls[LottoMachine.RETURN_BALL_COUNT];
                balls[x2] = tmp;
            }
        }
    }
